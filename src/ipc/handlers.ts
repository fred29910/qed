...
export function registerIpcHandlers(bus: IpcBus, ctx: HandlerContext): void {
    bus.register<undefined, ReturnType<ConfigService['snapshot']>>('config:get', () => ctx.config.snapshot());
    bus.register<ConfigUpdatePayload, ReturnType<ConfigService['snapshot']>>('config:update', (payload) => {
