...
**Generated:** 2026-09-03  Parent: ./AGENTS.md

## OVERVIEW

7 flat widget files, no `index.ts`. Theme palette helpers, sidebar navigation, status/title bars, always-on-top settings window, toast overlay, and small reusable widget primitives (`Section`, `Row`, `Badge`, `ButtonRow`). Uses `perry/ui` `State<T>` for local reactive state.

## STRUCTURE

```
src/ui/
├── theme.ts                # palette tokens + imperative paint helpers (paintMuted, paintText, etc.)
...
| Always-on-top settings window | `settings-window.ts` | Factory only. Controller owns the handle and reuses it (hide/show). |
...
- **Theme sync.** `applyTheme()` in `theme.ts` must be called when the store's theme changes. `settings-window.ts` is where the user toggles it. Both `state/app-state.ts` and `ui/theme.ts` must stay consistent.
